...
public class StreamAlgorithms {
  ...
  public static Stream<Rectangle> getAllIntersecting(
    ...
    return rectangles.filter(rectangle1 -> rectangle1.intersects(rectangle));
  }
  ...
  public static Stream<Rectangle> getAllWithBiggerAreaThan(
      Stream<Rectangle> rectangles, Rectangle rectangle) {
    return rectangles
            .filter(rectangle1 -> rectangle1.area() > rectangle.area());
  }
  ...
  public static Map<Rectangle, Integer> getAreaMap(Stream<Rectangle> rectangles) {
    return rectangles
           .collect(Collectors.toMap((m) -> m, Rectangle::area));
  }

  public static Optional<Rectangle> intersectAll(Stream<Rectangle> rectangles){
    return rectangles
            .map(Optional::of)
            .reduce((rectangle1, rectangle2) -> {
              if(rectangle1.isEmpty() || rectangle2.isEmpty())
                return Optional.empty();
              else
                return rectangle1.get().intersection(rectangle2.get());})
